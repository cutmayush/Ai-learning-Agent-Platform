...
### Gemini API Key
This project supports AI features (quizzes and personalized recommendations) via Google Gemini. To enable them you must set the `GENAI_API_KEY` environment variable with your API key. You can either set it in your shell or create a `.env` file at the project root with:

```
GENAI_API_KEY=your_gemini_api_key_here
```

`python-dotenv` is included in `requirements.txt` so `.env` will be loaded automatically. If `GENAI_API_KEY` is not set, the app will run normally but AI-powered features will be disabled and fallbacks will be used.

Note on compatibility: the `google-generative-ai` package (used for Gemini) may not have prebuilt wheels for newer Python versions (for example, Python 3.13). If `pip install google-generative-ai` fails on your system, use a supported Python version (3.11 or 3.12) or skip installing the package — the app will still run without AI features.
